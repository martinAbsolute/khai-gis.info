...
$(function() {
	...
	//gallery detail popup-Ann+1
	$('.sorting-item.open-popup1').on('click', function(){
		$('.gallery-popup1').addClass('active');
		var index = $(this).parent().find('.open-popup1').index(this);
		swipers['swiper-'+$('.gallery-popup').find('.swiper-container').attr('id')].swipeTo(index, 0);
		$('body, html').toggleClass('overflow-hidden');
	});

	$('.gallery-popup1 .close-popup').on('click', function(){
		$('.gallery-popup1').removeClass('active');	
		$('body, html').toggleClass('overflow-hidden');	
	});
